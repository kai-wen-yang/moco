...
def main_worker(gpu, ngpus_per_node, args):
    ...
    target_features = F.normalize(compute_features(target_loader, model, args)) # num target class, 128

    X = target_features
    label = target_dataset.targets
    with torch.no_grad():
        Y = TSNE(n_components=2, perplexity=30, n_iter=100, verbose=True).fit_transform(X)

    pyplot.scatter(Y[:, 0], Y[:, 1], 20, label)
    pyplot.show()
    pyplot.savefig('./target_feature_finetuned.jpg')
...
def compute_features(eval_loader, model, args):
    ...
    for i, (images, _, index) in enumerate(eval_loader):
