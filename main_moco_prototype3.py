...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.distributed or args.gpu==0:
        args.save_dir = './results/'+'rep3{}weightedsample{}'.format(args.replacement, args.temperature)+args.target_dataset
    ...
    features = torch.load('./results/features_v2.pt').to(torch.float16)
    ...
    data_weights = torch.exp((features.cuda()*centroids[im2cluster]).sum(dim=1)/args.moco_t) # init_data_weights(features, centroids)  #
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        data_weights, cluster_marginal = train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids_morph, data_weights, True, args, writer, scaler)
...
def train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids, data_weights, load_indices, args, writer, scaler):
    ...
    progress = ProgressMeter(
        args.train_iter,
        [batch_time, data_time, losses_p, losses_t],
    ...
    target_data_loader_iter = iter(target_loader)
    ...
    for batch_i in range(args.train_iter):
        try:
            ...
            inputs_p, index_p = data_loader_iter.next()
        try:
            inputs_t, label_t, _ = target_data_loader_iter.next()
        except:
            target_data_loader_iter = iter(target_loader)
            inputs_t, label_t, _ = target_data_loader_iter.next()
        ...
        if args.gpu is not None:
            ...
            inputs_t = inputs_t[0].cuda(args.gpu, non_blocking=True)
...
def init_data_weights(features, centroids):
    data_weights = torch.zeroes
