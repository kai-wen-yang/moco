...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.distributed or args.gpu==0:
        args.save_dir = './results/'+'temperature{}'.format(args.moco_t)+args.target_dataset
    ...
    target_loader = torch.utils.data.DataLoader(
        target_dataset, batch_size=64, shuffle=(target_sampler is None),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        data_conf, cluster_marginal = train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids_morph, True, args, writer, scaler)
        ...
        if epoch % args.select_freq == 0:
            ...
            centroids_morph = F.normalize(
                epoch / args.epochs * centroids_target + (1 - epoch / args.epochs) * centroids)
            _, temp_indices = torch.sort(data_conf, descending=True)
            data_num = int((1-epoch/args.epochs)*initial_num)
            select_temp_indices = temp_indices[:data_num]
            data_indices=data_indices[torch.LongTensor(select_temp_indices.cpu())]
            train_loader = torch.utils.data.DataLoader(
                moco.loader.SubSetInstance(train_dataset, data_indices), batch_size=args.batch_size,
                shuffle=(train_sampler is None),
                num_workers=args.workers, pin_memory=False, sampler=train_sampler)

            # draw matlotlib
            x = centroids_morph.mm(target_features.t()).sum(dim=1).cpu().numpy()
            y = cluster_marginal.cpu().numpy()
            plt.scatter(x, y, s=10, alpha=0.5)
            writer.log({'sim_num_/{}'.format(epoch): wandb.Image(plt)}, commit=False)
            plt.cla()

            # tsne plot
            X = torch.cat((centroids_morph[random_centroids], target_features), dim=0)
            with torch.no_grad():
                X = TSNE(n_components=2, perplexity=30, n_iter=100, verbose=True).fit_transform(X)
            Y = torch.cat((torch.zeros(centroids_morph[random_centroids].size(0)),torch.ones(target_features.size(0))), dim=0)
            size = torch.cat(((cluster_marginal[random_centroids]/cluster_marginal.mean()*10).cpu(), torch.ones(target_features.size(0))*10), dim=0)
            plt.scatter(X[:, 0], X[:, 1], size, Y)
            plt.savefig(os.path.join(args.save_dir,'tsne_{}.png'.format(epoch)))
            writer.log({'tsne/{}'.format(epoch): wandb.Image(plt)}, commit=False)
            plt.cla()
        ...
        if writer:
            writer.log({'test/acc' :acc1,
                        ...
                        'cluster_margin/{}'.format(epoch): wandb.Histogram(cluster_marginal.cpu().detach().numpy(), num_bins=512),}, commit=False)
...
def train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids, load_indices, args, writer, scaler):
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses_p, losses_t, top1, top5],
        prefix="Epoch: [{}]".format(epoch))

    # switch to train mode
    model.train()
    if load_indices:
        data_conf = torch.zeros(len(train_loader.dataset)).cuda()
        cluster_marginal = torch.zeros(args.num_cluster).cuda()
    end = time.time()
    target_loader_iter = iter(target_loader)

    for batch_i, (inputs_p, index_p) in enumerate(train_loader):
        try:
            inputs_t, label_t, _ = target_loader_iter.next()
        except:
            target_loader_iter = iter(target_loader)
            inputs_t, label_t, _ = target_loader_iter.next()
        ...
        with torch.cuda.amp.autocast():
            # compute output
            output_p, sim, target_p, max_probs = model(im_q=inputs_p[0], im_k=inputs_p[1], centroids=centroids, temp=args.moco_t)
            output_t, _, target_t, _ = model(im_q=inputs_t[0], im_k=inputs_t[1], centroids=centroids, temp=args.moco_t)
        ...
        if writer:
            writer.log({'train/loss_p': losses_p.avg,
                        'train/loss_t': losses_t.avg,
                        'train/acc1': top1.avg,
                        'train/mean_confidence': max_probs.mean(),
                        'max_probs/{}'.format(epoch): wandb.Histogram(max_probs.cpu().detach().numpy(), num_bins=512)})

        if batch_i % args.print_freq == 0:
            progress.display(batch_i)
        if load_indices:
            data_conf[index_p] = max_probs
            cluster_marginal += F.one_hot(target_p, num_classes=centroids.size(0)).sum(dim=0)
    if load_indices:
        return data_conf, cluster_marginal
