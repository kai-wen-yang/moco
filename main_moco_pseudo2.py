...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.gpu==0:
        ...
        torch.save(features, './results/features_v2.pt')
        features = features.numpy()
        cluster_result = run_kmeans(features, args)  # run kmeans clustering on master node
        torch.save(cluster_result, './results/cluster_result_v2')
        ...
        selected_centroids = torch.LongTensor(random.sample(range(centroids.size(0)), int(args.num_cluster*0.1)))#indices[:int(args.num_cluster*0.1)]  # cluster num
        select = torch.Tensor(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.select_freq == 0 and epoch != 0:
            data_indices = data_indices[data_conf.cpu()]
            train_loader = torch.utils.data.DataLoader(
...
def train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids, im2cluster, density, load_indices, args, writer, scaler):
    ...
    model.train()
    if load_indices:
        data_conf = torch.zeros(len(train_loader.dataset)).bool().cuda()
        cluster_marginal = torch.zeros(len(train_loader.dataset), 2000).cuda()
    end = time.time()
    ...
    for batch_i, (inputs_p, index_p) in enumerate(train_loader):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            loss = loss_p  + 0* loss_t

        selected_index = index_p[max_probs.ge(args.threshold)]
        ...
        if load_indices:
            data_conf[index_p] = max_probs.ge(args.threshold)
