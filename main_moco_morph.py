...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.distributed or args.gpu==0:
        writer = wandb.init(config=args, name='dataselect'+args.target_dataset)
    else:
        writer = None
    ...
    if args.gpu==0:
        ...
        similarity_norm = similarity / similarity_sum.unsqueeze(-1)
        centroids_target = similarity_norm.mm(target_features)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        centroids_morph = epoch/args.epochs * centroids_target + (1-epoch/args.epochs) * centroids
        # train for one epoch
        train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids_morph, im2cluster, args, writer, scaler)
