...
import moco.loader
import moco.builder_morph_pesudo
import pdb
import faiss
from utils import *
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder_morph_pesudo.MoCo(
    ...
    if not args.distributed or args.gpu==0:
        args.save_dir = './results/'+'selectedcentroids{}'.format( args.temperature)+args.target_dataset
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=128, shuffle=(train_sampler is None),
        num_workers=args.workers, pin_memory=True, sampler=train_sampler)
    ...
    if args.gpu==0:
        ...
        initial_num = len(data_indices)
        data_num = initial_num
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        data_conf, cluster_marginal = train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids_morph[selected_centroids], im2cluster, density[selected_centroids], True, args, writer, scaler)
        acc1 = validate(val_loader, model, criterion, args)

        if epoch % args.select_freq == 0 and epoch != 0:
            target_features = F.normalize(compute_target_features(target_loader, model, args))  # num target class, 128
            similarity = centroids.mm(target_features.t())  # cluster num, class num
            ...
            selected_centroids = selected_centroids[cluster_marginal>(data_num/(selected_centroids.size(0)*3))]
            ...
            data_indices=data_indices[select_temp_indices.cpu()]
            train_loader = torch.utils.data.DataLoader(
...
def train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids, im2cluster, density, load_indices, args, writer, scaler):
    ...
    if load_indices:
        ...
        cluster_marginal = torch.zeros(len(train_loader.dataset), centroids.size(0)).cuda()
    end = time.time()
    ...
    for batch_i, (inputs_p, index_p) in enumerate(train_loader):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            output_p, target_p, max_probs = model(im_q=inputs_p[0], im_k=inputs_p[1], centroids=centroids, index=index_p, im2cluster=im2cluster, density=density,temp=args.temperature)
        ...
        if load_indices:
            ...
            cluster_marginal[index_p] += F.one_hot(target_p, num_classes=centroids.size(0))
    if load_indices:
        return data_conf, cluster_marginal.sum(dim=0)
