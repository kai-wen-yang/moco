...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.gpu==0:
        ...
        cluster_result = torch.load( './results/cluster') #
        #cluster_result = run_kmeans(features, args)  # run kmeans clustering on master node
        #torch.save(cluster_result, './results/cluster_'+args.target_dataset)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids, im2cluster, args, writer, scaler)
...
def train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids, im2cluster, args, writer, scaler):
    ...
    for batch_i, (inputs_p, index_p) in enumerate(train_loader):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            output_p, target_p, max_probs = model(im_q=inputs_p, centroids=centroids, im2cluster=im2cluster, index=index_p)
