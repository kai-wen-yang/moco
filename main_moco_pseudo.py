...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.distributed or args.gpu==0:
        writer = wandb.init(config=args, name='alldata_taegetcentroid'+args.target_dataset)
    else:
        writer = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        centroids_morph = F.normalize(epoch/args.epochs * centroids_target + (1-epoch/args.epochs) * centroids)
        # train for one epoch
        data_indices = train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids_morph, True, args, writer, scaler)
        acc1 = validate(val_loader, model, criterion, args)

        if writer:
            writer.log({'test/acc' :acc1,
                        'data_num': data_indices.sum()}, commit=False)
...
def train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids, load_indices, args, writer, scaler):
    ...
    model.train()
    if load_indices:
        data_indices = torch.zeros(len(train_loader.dataset)).bool().cuda()
    end = time.time()
    ...
    for batch_i, (inputs_p, index_p) in enumerate(train_loader):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            output_p, target_p, max_probs = model(im_q=inputs_p, centroids=centroids, index=index_p)
        ...
        if load_indices:
            data_indices[index_p] = max_probs.ge(args.threshold)
    if load_indices:
        return  data_indices
