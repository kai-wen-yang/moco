...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.distributed or args.gpu==0:
        writer = wandb.init(config=args, name='randomcentroid_temp{}'.format(args.temperature)+args.target_dataset)
    else:
        writer = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        data_conf, cluster_marginal = train(train_loader, target_loader, model, criterion, optimizer, epoch, centroids[selected_centroids], im2cluster, density[selected_centroids], True, args, writer, scaler)
